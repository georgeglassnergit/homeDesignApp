...
| A4 · Auto room detection | in-progress (auto/2026-08-11) — **pure engine landed** | pure `detectRooms`/`detectNewRooms` in **new** `src/core/roomDetect.js` (planar wall-graph → bounded-face cycles; endpoint-merge + T-junction split; spurs/dupes dropped; derived-on-read, no save field). New `src/test/room-detect.test.mjs` **29/29**; regressions phase2 33 / edit-core 337 / roof 40 / element-label 41 / gable 25 / hover 15 / rename 18 green. New files only → **zero source collision** with open PRs #24–27. UI wiring (plan affordance, reuse `renameRoom`) is the follow-up once the pile merges & `planCanvas`/`main.js`/`state.js` are free. |
