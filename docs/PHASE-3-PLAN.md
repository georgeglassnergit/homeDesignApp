...
| A4 · Auto room detection | PR #29 (integration, auto/2026-08-15) — pure engine landed; supersedes #28 | pure `detectRooms`/`detectNewRooms` in **new** `src/core/roomDetect.js` (planar wall-graph → bounded-face cycles; endpoint-merge + T-junction split; spurs/dupes dropped; derived-on-read, no save field). `room-detect.test.mjs` **29/29**. UI wiring (plan affordance, reuse `renameRoom`) is the follow-up now the source files are free. |
