...
**Still dormant** (registered in `FEATURE_TIERS` but *not* wired to any UI): `furnish-photo-3d` (F1 — needs a server-side key, scheduled last). `materials-swatch`, `code-checks`, `ifc-export` are now **live** (E1 / E2 / E3). **No disabled "coming soon" tiles remain** — both onboarding tiles (*Import a plan* D1, *Outsource* D2) are wired.
...
| Outsource intake | D2 | ✅ | ✅ | `outsource` (un-tiered) — **live** |
...
| D2 · Outsource-drawing UX | Landed (auto/2026-08-25 integration) — engine + UI | new pure `src/core/outsourceRequest.js` (imports only `model.js`+`units.js`, **zero Three.js**): `buildProjectBrief(project,{units})` derives a shareable summary — storey/room/wall/opening counts, GFA, per-room area+perimeter, materials-used, roof-types — reusing the inspector's own read helpers; `formatBriefText` renders a deterministic plain-text brief; `SERVICE_LEVELS` + `createOutsourceRequest`/`validateOutsourceRequest`/`outsourceRequestJson` assemble a downloadable request (needs a contact; email sanity; unknown service → default). Everything is DERIVED-ON-READ — no upload, no API key (constraint #7), no save field, mutates nothing. The "Outsource" onboarding tile is now LIVE: clicking it opens a local, no-backend modal (auto-generated brief preview + contact/service/notes form + Copy / Download `.txt` / Download request `.json`), always showing the advisory `OUTSOURCE_DISCLAIMER` (constraint #6 — the third-party drafter, not this tool, certifies drawings). New un-tiered `outsource` seam. `outsource-request.test.mjs` **56/56**. Headless **26/26**, zero console errors, screenshot `docs/verification/phase3-outsource.png`. **Both onboarding tiles (Import a plan + Outsource) are now wired — no "coming soon" tiles remain.** |
