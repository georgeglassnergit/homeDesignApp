...
**Still dormant** (registered in `FEATURE_TIERS` but *not* wired to any UI): `furnish-photo-3d` (F1, server-side, deferred). `materials-swatch`, `code-checks`, `ifc-export` are all now live (E1/E2/E3). **Onboarding tiles all live:** *Import a plan* (D1) and *Outsource* (D2) — no "coming soon" tiles remain.
...
| C1 · Angled walls | Landed (auto/2026-09-07 integration) — Pro angle entry | walls already store free `a/b` points and the C2 mitre view already renders any angle, so C1 is the **entry**: classic CAD **direct distance/angle**. New pure `edit/snapping.js` helpers `polarOffset`/`segmentPolar`/`normalizeAngleDeg` (plan-frame **0°=east, 90°=up**, matching the DXF `(x,z)→(x,−z)` map); `ToolController.polarEntry(len,angleDeg)` commits the next corner at the exact offset via the same `createWall→addWall→history` path (undoable, no save field), chaining from the last point; `currentSegmentPolar()` drives a live readout. UI: a "Precise segment" block in the existing Pro snap panel (reuses the `snapping-constraints` seam — no new seam), units-aware, Enter-to-commit. Zero Three.js; separation guard green. `angle-entry.test.mjs` **60/60**. Headless `c1-angle-entry.mjs` **23/23, zero console errors** — a 3-4-5 triangle typed by polar segments (hypotenuse off every 90° axis), Pro-only seam, exact endpoints vs the pure oracle, chaining, undo, and a **CSG window surviving on the angled wall** (hole → 64 tris vs a plain 12-tri prism) with the save still lossless. Screenshot `docs/verification/phase3-angle-entry.png`. **Group C complete (C1 + C2).** |
| C2 · Merged / T-junction walls | Landed (auto/2026-09-05) — engine + view wiring | **View wiring (2026-09-05):** `build/geometry.js` now extrudes each wall's mitred quad from `joinWalls` (via `build/sceneBuilder.js`, which computes the join per level) into a prism instead of a plain centreline box, then subtracts openings via the same CSG path. A wall with no mitre (a free end, or the sample partition) extrudes its plain rectangle — byte-identical footprint to the old box, so a lone wall is unchanged; a zero-length wall still falls back to the legacy box. Pure `wallJoin.js` unchanged (still core-only, no Three.js). Headless `c2-wall-mitre.mjs` **12/12, zero console errors**: all four outer corners of the sample home extrude their mitred footprints (matching the engine), the plain-box end corner is gone, adjoining walls share the SAME seam points (gap-free & non-overlapping), the partition keeps square ends, and the four CSG openings survive (a hole → more triangles than the no-opening west wall's 12-tri prism). Screenshot `docs/verification/phase3-wall-mitre.png`. **Prior (2026-08-20):** | pure `core/wallJoin.js` (new file, imports only `model.js`, no Three.js): `joinWalls(walls)` → per-wall mitred quad `[aR,bR,bL,aL]`. At a node where EXACTLY two walls meet, their ±½-thickness offset edges are intersected so the rectangles share ONE seam (gap-free **and** non-overlapping); collinear edges, free ends, and T/X (3+) nodes stay square (safe watertight fallback); a runaway acute spike is capped by a mitre limit. Derived-on-read — mutates nothing, no save field. `wall-join.test.mjs` **44/44**. View wiring (extrude the quads into corner prisms in `build/geometry.js`) is the follow-up once `main.js`/`geometry.js` are free of the open UI PRs. |
...
| D2 · Outsource-drawing UX | Landed (auto/2026-09-04) — engine + UI | wires the last disabled onboarding tile. New pure `core/outsourceBrief.js` (imports only `model.js`+`units.js`, zero Three.js) composes a plain-text **design brief** from the current model + optional intake fields (name/email/phone/style/timeline/notes) a homeowner can hand to a professional — project overview (storeys, rooms, GFA, per-storey breakdown, wall/opening/furniture counts) + their notes, always footered "approximate homeowner estimate, not a construction spec". No backend (content/UX only), no save field, mutates nothing. `main.js` enables the "Outsource" tile → an intake modal that previews the live brief, copies to clipboard, and downloads `home-design-brief.txt`. Un-tiered. `outsource-brief.test.mjs` + headless. |
...
| E2 · Advisory checks (advisory only) | Landed (auto/2026-08-22 integration) — engine + UI | pure `src/core/advisoryChecks.js` (`runAdvisoryChecks` + per-check fns) — reads walls/openings/rooms/level heights, flags approximate residential rules of thumb (ceiling height, habitable room area, door width/height, egress window area/sill, storey natural-light/access) as **advisory-only** findings with an always-present `ADVISORY_DISCLAIMER`. Computed on read; no save field; mutates nothing; imports only `model.js`. A Pro "Checks…" popover (`code-checks` seam) lists the findings by location; clicking one selects the element it names (opening→that opening, storey→that storey) as pure view state (no command). `advisory-checks.test.mjs` **46/46**. Headless 21/21, screenshot `docs/verification/phase3-advisory-checks.png`. |
...
| E3+ · DXF 2D floor-plan export | Landed (auto/2026-09-07 integration) — engine + UI | pure `core/exportDxf.js` — a **DXF R12 (AC1009)** 2D CAD plan (the interchange OBJ/glTF/JSON don't provide; what a drafter/architect opens first, and what the D2 outsource brief travels with). Layer-organised per storey (`{TAG}-WALLS/-OPENINGS/-ROOMS/-ROOM-LABELS`). Walls REUSE `joinWalls` (mitred footprint — 2D can't drift from 3D); openings as hole rectangles; rooms as boundary + centroid `TEXT` (name + area, units-aware); `(x,z)→(x,−z)` so plan-up = CAD-up. Imports only `wallJoin.js`+`model.js`+`units.js`, zero Three.js; derived-on-read, no save field, mutates nothing. `export-dxf.test.mjs` **39/39** (incl. a self-contained structural DXF validator + byte-identical-save assertion). A "Download DXF (2D plan)" item added to the existing Export… popover (`main.js`/`index.html`, same Pro `ifc-export` seam), units-aware via `app.units`. Headless **18/18** (`probes/e3plus-dxf.mjs`), screenshot `docs/verification/phase3-integration-e3plus-dxf.png`. Visual `docs/verification/phase3-export-dxf.svg`. |
| E3+ · glTF 2.0 export | Landed (auto/2026-08-22 integration) — engine + UI | pure `core/exportGltf.js` — the third E3 format the plan names ("glTF/OBJ/JSON"). REUSES `exportObj.js`'s `wallMesh`/`roomSlabMesh` massing helpers (so glTF & OBJ can't drift) and emits a single self-contained glTF 2.0 doc: one JSON with the binary geometry buffer embedded as a base64 `data:` URI (no companion `.bin`), PBR materials from the project's material map. Portable base64 encoder (no Buffer/btoa) → runs in Node **and** the browser; imports only `exportObj.js`+`model.js`, **zero Three.js**. Derived-on-read; mutates nothing; no save field. `export-gltf.test.mjs` **47/47** + real **three.js GLTFLoader** round-trip. A "Download glTF 2.0" item added to the existing Export… popover. Headless 17/17, screenshot `docs/verification/phase3-export-gltf.png`. |
